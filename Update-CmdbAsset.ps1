...
.PARAMETER wikiUrl
An optional Wiki documentation URL for the asset.
...
function Update-CmdbAsset {
    ...
    param(
        ...
        [Parameter(Mandatory = $false, HelpMessage = "The display name of the asset when adding or updating.")]
        [string]$Name,
        
        [Parameter(Mandatory = $false, HelpMessage = "The support team or group responsible for the asset.")]
        [string]$SupportGroup,
        
        [Parameter(Mandatory = $false, HelpMessage = "An optional wiki documentation URL for the asset.")]
        [string]$wikiUrl,
        
        [Parameter(Mandatory = $false, HelpMessage = "An optional management console URL for the asset.")]
        [string]$ManagementUrl,

        [Parameter(Mandatory = $false, HelpMessage = "Environment for the asset.")]
        [ValidateSet('production', 'staging', 'development', 'lab')]
        [string]$Environment,

        [Parameter(Mandatory = $false, HelpMessage = "Service criticality tier.")]
        [ValidateSet('critical', 'high', 'medium', 'low', 'platinum', 'gold', 'silver', 'bronze')]
        [string]$Tier,
        
        [Parameter(Mandatory = $false, HelpMessage = "An optional IP address for the asset.")]
        [string]$Ip,
        ...
        switch ($Action) {
            'Add' {
                ...
                $NewAsset = [PSCustomObject]@{
                    ...
                    wiki_url = if ($PSBoundParameters.ContainsKey('wikiUrl')) { $wikiUrl } else { "UpdateME" }
                    management_url = if ($PSBoundParameters.ContainsKey('ManagementUrl')) { $ManagementUrl } else { "UpdateME" }
            ...
            'Update' {
                ...
                if ($PSCmdlet.ShouldProcess("Asset ID: $Id", "Update existing asset fields")) {
                    ...
                    if ($PSBoundParameters.ContainsKey('wikiUrl')) { $ExistingAsset.wiki_url = $wikiUrl }
